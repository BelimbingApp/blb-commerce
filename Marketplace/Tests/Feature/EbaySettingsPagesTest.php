...
test('eBay settings imports seller setup choices and stores selected defaults', function (): void {
    ...
    $settings->set('commerce.marketplace.ebay.client_secret', 'secret-setup-import', $scope);
...
test('eBay settings opts in, creates a merchant location, and creates starter policies', function (): void {
    ...
    $settings->set('commerce.marketplace.ebay.client_secret', 'secret-acct-actions', $scope);
...
test('eBay settings updates an existing location address instead of failing or duplicating', function (): void {
    ...
    $settings->set('commerce.marketplace.ebay.client_secret', 'secret-loc-update', $scope);
...
test('eBay settings saves a manual category mapping instantly and resolves the tree', function (): void {
    ...
    $settings->set('commerce.marketplace.ebay.client_secret', 'secret-manual-test', $scope);
...
test('eBay settings finds categories by name and applies the chosen suggestion', function (): void {
    ...
    $settings->set('commerce.marketplace.ebay.client_secret', 'secret-suggest-test', $scope);
...
test('eBay client secret field can reveal the saved Cert ID when configured', function (): void {
    ...
    app(SettingsService::class)->set('commerce.marketplace.ebay.client_secret', 'client-secret-1234567890', $scope);
...
test('eBay OAuth authorize URL uses the eBay RuName instead of the callback URL', function (): void {
    ...
    $settings->set('commerce.marketplace.ebay.client_secret', 'secret-oauth-url', $scope);
    $settings->set('commerce.marketplace.ebay.ru_name', 'KiatNg-Belimbin-SBX-runame', $scope);
...
test('eBay settings diagnostics verifies the saved OAuth grant against a safe Account API call', function (): void {
    ...
    $settings->set('commerce.marketplace.ebay.client_secret', 'secret-connection-test', $scope);
    $settings->set('commerce.marketplace.ebay.ru_name', 'KiatNg-Belimbin-SBX-runame', $scope);
    $settings->set('commerce.marketplace.ebay.scopes', [
        EBAY_SCOPE_ACCOUNT,
        EBAY_SCOPE_INVENTORY,
...
test('eBay settings diagnostics explains when OAuth has not been connected yet', function (): void {
    ...
    $settings->set('commerce.marketplace.ebay.client_secret', 'secret-without-oauth', $scope);
    $settings->set('commerce.marketplace.ebay.ru_name', 'KiatNg-Belimbin-SBX-runame', $scope);
    $settings->set('commerce.marketplace.ebay.scopes', [
        EBAY_SCOPE_ACCOUNT,
        EBAY_SCOPE_INVENTORY,
...
function seedConnectedEbay(User $user, array $scopes): void
{
    ...
    $settings->set('commerce.marketplace.ebay.client_secret', 'secret-diagnostics', $scope);
    $settings->set('commerce.marketplace.ebay.ru_name', 'KiatNg-Belimbin-SBX-runame', $scope);
